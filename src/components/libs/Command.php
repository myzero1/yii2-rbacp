...
class Command extends Component
{
     ...
     * @since 2.0.6
     */
    protected function createOperationLog()
    {
        ...
        if (is_array($aOperationLogTemplate) && array_key_exists($sCurrentRoute, $aOperationLogTemplate) && !$aOperationLogTemplate[$sCurrentRoute]['useTransaction']) {
            if (strpos($sql, 'SELECT') === 0) {
                # code
            } else {

                $mTablePos = strpos($sql, $aOperationLogTemplate[$sCurrentRoute]['identifyingTable']);
                //保证sql中的主表为特征表
...
var_dump($sHtml);exit;
                    ...
                    $sql = sprintf('INSERT INTO log_operation (id, user_id, user_name, operation, ip, snapshoot, created, updated_table, url) VALUES(NULL, %d, "%s", "%s", "%s", "%s", %d, "%s", "%s")',
                        ...
                        base64_encode($sHtmlCom),
                        time(),
                        $aOperationLogTemplate[$sCurrentRoute]['identifyingTable'],
                        \Yii::$app->request->url
                    );
                    $this->sOperationSql = $sql;
                } else {
                    # coding
                }
    ...
    protected function saveOperationLog()
    {
        ...
        if ($this->sOperationSql) {
            if (\Yii::$app->params['addOperationLog']) {
                ...
                \Yii::$app->db->createCommand($this->sOperationSql)->execute();
            } else {
                # code...
            }
        } else {
