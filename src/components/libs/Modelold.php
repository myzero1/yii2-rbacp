...
class Model extends Component implements IteratorAggregate, ArrayAccess, Arrayable
{
    ...
    {
        $aAreaMap = \Yii::$app->params['logConfig']['params']['aAreaMap'];

        foreach ($data as $key => $value) {
            if (array_key_exists($key, $aAreaMap)) {
                $mArea = $data[$key][$aAreaMap[$key]];
                if (is_array($mArea)) {
                    // Yii::$app->params['sLogArea'] = sprintf(',%s,', implode(',', $mArea));
                    Yii::$app->params['mLogArea'] = $mArea;
                } else {
                    // Yii::$app->params['sLogArea'] = sprintf(',%s,', $mArea);
                    Yii::$app->params['mLogArea'] = explode(',', trim($mArea, ','));
                }
                // var_dump(Yii::$app->params['sLogArea']);exit;
            } else {
                // Yii::$app->params['mLogArea'] = array(-1);
                // Yii::$app->params['mLogArea'] = explode(',', Yii::$app->user->identity->auth_zones);
            }
        }

        // added by qinxianwu,当发起请求是post但需要用runAction获取get请求的html
        // var_dump(Yii::$app->params['onlyContentHtml']);exit;
        if (Yii::$app->params['onlyContentHtml']) {
            return FALSE;
        } else {
            # code...
        }
