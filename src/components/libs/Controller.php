...
class Controller extends \yii\base\Controller
{
    ...
    {
        //在这里初始化功能权限控制,先要判断是的从web发过来的请求
        $appClassName = get_class( \Yii::$app );
        if ($appClassName=='yii\web\Application') {
            if (\rbacp\components\Rbacp::requestPrivilege() == 403) {
                if(!$action instanceof \yii\web\ErrorAction) {
                    throw new ForbiddenHttpException(Yii::t('yii', 'You are not allowed to perform this action.'));
                }
            } else {
                # 执行参数策略
                if (\rbacp\components\Rbacp::paramPrivilege()) {
                    # code...
                } else {
                    if(!$action instanceof \yii\web\ErrorAction) {
                        throw new ForbiddenHttpException(Yii::t('yii', 'You are not allowed to perform this action.'));
